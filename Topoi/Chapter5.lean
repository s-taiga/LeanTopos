...
lemma χ.w : f ≫ χ f hf = terminal.from _ ≫ true (Ω := Ω) := by
...
abbrev fFork := Fork.ofι
  ...
  f (by rw [χ.w f hf (Ω := Ω), ← assoc, terminal.comp_from])

def «Theorem 1» : IsLimit (fFork f hf (Ω := Ω)) := by
  have ⟨h, _⟩ := χ.spec f hf (Ω := Ω)
  let _lift (c : Fork (χ f hf (Ω := Ω)) (terminal.from b ≫ true)) : c.pt ⟶ a := h.lift c.ι (terminal.from c.pt) (by
    have := c.condition
    dsimp at this
    rw [this, ← assoc, terminal.comp_from])
  apply Fork.IsLimit.mk (fFork f hf) _lift
  . intro c
    dsimp [_lift]
    rw [h.lift_fst]
  intro c hm hmι
...
theorem «Corollary» (Ω : 𝓒) [ElementaryTopos Ω] : IsIso f ↔ Epi f ∧ Mono f := by
  ...
  intro ⟨hepi, hmono⟩
  have := «Theorem 1» f hmono (Ω := Ω)
  have eq : f = (fFork f hmono (Ω := Ω)).ι := rfl
  rw [eq] at *
...
def «Exercise» : IsLimit (tFork (Ω := Ω)) := by
  ...
  -- have hp := «Theorem 1» true ((«§3.6.ex3» true))
  -- TODO: Forkの型が違うからできない？
  -- apply IsLimit.ofIsoLimit hp (t := tFork sc)
  sorry
...
theorem «Theorem 1» {c : 𝓒} (Ω : 𝓒) [ElementaryTopos Ω] (u : a ⟶ c) (v : c ⟶ b) (hv : Mono v) (huv : u ≫ v = f) :
  ∃! k : fa ⟶ c, fstar hf ≫ k = u ∧ k ≫ v = imf := by
  have :=  «§5.1».«Theorem 1» v hv (Ω := Ω)
  let s := χ v hv (Ω := Ω)
  let t := «CH.4».«§4.2».true' b (Ω := Ω)
  let vcone := «§5.1».fFork v hv (Ω := Ω)
  ...
  let k : fa ⟶ c := Fork.IsLimit.lift («§5.1».«Theorem 1» v hv) (equalizer.ι p q) this
  ...
  intro k' ⟨hk'₁, hk'₂⟩
  apply Fork.IsLimit.hom_ext («§5.1».«Theorem 1» v hv (Ω := Ω))
  dsimp
  rw [hk'₂, hk]

theorem Corollary [ElementaryTopos Ω] : Epi (fstar hf) := by
  let ga : 𝓒 := equalizer (imf ≫ p) (equalizer.ι p q ≫ q)
  let img : ga ⟶ equalizer p q := equalizer.ι (imf ≫ p) (imf ≫ q)
  have hg : IsPushout (fstar hf) (fstar hf) (imf ≫ p) (imf ≫ q) := by
    sorry
  let gstar : a ⟶ ga := equalizer.lift (fstar hf) hg.w
  have h : gstar ≫ img = fstar hf := by
    rw [equalizer.lift_ι]
  have : Mono img := by
    apply «CH.3».«§3.10».«theorem 1»
...
theorem «Theorem 2» {c : 𝓒} (Ω : 𝓒) [ElementaryTopos Ω] (u : a ⟶ c) (v : c ⟶ b) (hu : Epi u) (hv : Mono v) (huv : u ≫ v = f):
  ∃! k : fa ⟶ c, fstar hf ≫ k = u ∧ k ≫ v = imf ∧ IsIso k := by
  have ⟨k, ⟨hk₁, hk₂⟩, huniq⟩ := «Theorem 1» hf Ω u v hv huv
  use k
  constructor
  . constructor
    apply hk₁
    ...
    have kmono : Mono k := by
      ...
      apply himf
    have kepi : Epi k := by
      apply «CH.3».«§3.1».Excercises.«(2)'» 𝓒 (fstar hf) k
      rw [hk₁]
      apply hu
...
theorem «Exercise 4» [ElementaryTopos Ω] : Epi f ↔ ∃ g : fa ≅ b, (fstar hf) ≫ g.hom = f := by
  constructor
  . intro hepi
    have ⟨k, ⟨hk₁, hk₂, hkiso⟩, hkuniq⟩ := «Theorem 2» hf Ω f (𝟙 b) hepi inferInstance (by rw [comp_id])
    rw [comp_id] at hk₂
    rw [hk₂] at hk₁ hkiso
    use asIso imf
...
axiom «Fact 1» {a b c d : 𝓒} {f₁ : a ⟶ b} {f₂ : c ⟶ d} {f : b ⟶ d} {g : a ⟶ c}
  (h : IsPullback f₁ g f f₂) (hf: Epi f) : Epi g
...
def «Extensionality Principle For Arrows» {a b : 𝓒} (f g : a ⟶ b) : Prop := ∀ (h : f ≠ g), ∃ x : element a, x ≫ f ≠ x ≫ g

variable (𝓒)
abbrev degenerate := ∀ a b : 𝓒, a ≅ b
abbrev nonDegenerate := ¬ IsIso (initial.to (⊤_ 𝓒))
variable {Ω : 𝓒}
...
theorem «Theorem 1» [WellPointed 𝓒] [ElementaryTopos Ω] : ∀ a : 𝓒, nonZero a → nonEmpty a := by
  intro a ha
  let za : ⊥_ 𝓒 ⟶ a := initial.to _
  let oa : a ⟶ a := 𝟙 _
  have hza : Mono za := («CH.3».«§3.16».«Theorem 1».«(4)» za)
  have hoa : Mono oa := inferInstance
  have : ¬ za ≃ₛ oa := by
    intro ⟨⟨f₁, hf₁⟩, ⟨f₂, hf₂⟩⟩
    dsimp [oa, za] at *
    rw [comp_id] at hf₁
    ...
    . rw [initial.to_comp]
      ext
    apply hf₂
  have : χ za hza ≠ χ oa hoa := «CH.4».«§4.2».Theorem za hza oa hoa (Ω := Ω) |>.not.mp this
  have ⟨x, hx⟩ := WellPointed.epfa' _ (χ za hza) (χ oa hoa) this
  constructor
  use x

lemma hMonoIni {a : 𝓒} : Mono (initial.to a) := «CH.3».«§3.16».«Theorem 1».«(4)» _

abbrev false' : ⊤_ 𝓒 ⟶ Ω := χ (initial.to _) hMonoIni

abbrev false (a : 𝓒) : a ⟶ Ω := terminal.from a ≫ false'

-- IsPullback (initial.to a) (terminal.from (⊥_ 𝓒)) (false a) true
theorem «Exercise 3» (a : 𝓒) : χ (initial.to a) hMonoIni = false a (Ω := Ω) := by
  have ⟨_, huniq⟩ := χ.spec (initial.to a) hMonoIni (Ω := Ω)
  symm
  apply huniq <| false a
  dsimp [false]
  ...
  have ⟨hPB, _⟩ := χ.spec (initial.to (⊤_ 𝓒)) hMonoIni (Ω := Ω)
  have : initial.to (⊤_ 𝓒) = terminal.from (⊥_ 𝓒) := by ext
  rw [← this] at hPB
  apply hPB

lemma lemma1 : true (Ω := Ω) = χ (𝟙 (⊤_ 𝓒)) inferInstance := by
  have ⟨hPB, huniq⟩ := χ.spec (𝟙 (⊤_ 𝓒)) inferInstance (Ω := Ω)
  apply huniq
  rw [«CH.3».«§3.6».terminal_id]
  apply «CH.3».«§3.13».«Example 9» 𝓒 true |>.mp («CH.3».«§3.6».Excercises.«3» _ _)

theorem «Exercise 4» [WellPointed 𝓒] : true (Ω := Ω) ≠ false' := by
  intro h
  apply WellPointed.nd' 𝓒
  constructor
  dsimp [false'] at h
  rw [lemma1] at h
  have ⟨⟨f, hf⟩, ⟨finv, hfinv⟩⟩ := «CH.4».«§4.2».Theorem (𝟙 (⊤_ 𝓒)) inferInstance (initial.to (⊤_ 𝓒)) hMonoIni |>.mpr h
  use f
  constructor
  . rw [initial.to_comp]
    ext
...
variable (𝓒)
abbrev bivalent [ElementaryTopos Ω] (nd : nonDegenerate 𝓒) : Prop := ∀ f : element Ω, f = true (Ω := Ω) ∨ f = false'
variable {𝓒}

theorem «Theorem 2» [ElementaryTopos Ω] [WellPointed 𝓒]: bivalent 𝓒 (WellPointed.nd' 𝓒) (Ω := Ω) := by
  intro f
  have ⟨a, g, ⟨hmono, hPB⟩, huniq⟩ := «CH.4».«§4.2».Ω_axiom' f
  let χf := χ g hmono (Ω := Ω)
  have ⟨hfPB, hfuniq⟩ := χ.spec g hmono (Ω := Ω)
  ...
  . have ia : IsInitial a := IsInitial.ofIso initialIsInitial <| asIso (initial.to a)
    ...
    apply «CH.4».«§4.2».Theorem g hmono (initial.to (⊤_ 𝓒)) hMonoIni (Ω := Ω) |>.mp
    rw [ia.hom_ext g (ia.to _)]
    constructor
    . use ia.to _
      rw [ia.to_comp]
    use initial.to _
  ...
  have hgepi : Epi g := by
    constructor
    intro b h k hg
    have hg : x ≫ g ≫ h = x ≫ g ≫ k := by congr
    have : x ≫ g = 𝟙 _ := by ext
    rw [← assoc, ← assoc, this, id_comp, id_comp] at hg
    apply hg
  have hgmono : Mono g := by
    apply «CH.3».«§3.13».Exercise _ _ _ _ _ hPB.flip
    apply «CH.3».«§3.6».Excercises.«3»
  have := «§5.1».Corollary g Ω |>.mpr ⟨hgepi, hgmono⟩
  let g' : a ≅ ⊤_ 𝓒 := asIso g
  have ta : IsTerminal a := IsTerminal.ofIso terminalIsTerminal g'.symm
  left
  apply hfuniq f hPB |>.trans
  rw [lemma1]
  apply «CH.4».«§4.2».Theorem g hmono (𝟙 (⊤_ 𝓒)) inferInstance (Ω := Ω) |>.mp
  constructor
  . use g'.hom
...
abbrev disjointArr : Prop := IsPullback (initial.to c) (initial.to a) g f

variable {f} {g}
theorem «Lemma» (hf : Mono f) (hg : Mono g) (hd : disjointArr f g) : Mono (coprod.desc f g) := by
...
theorem «Theorem 3» [ElementaryTopos Ω] : Mono (coprod.desc (true (Ω := Ω)) false') := by
  have : IsPullback (initial.to (⊤_ 𝓒)) (initial.to (⊤_ 𝓒)) false' (true (Ω := Ω)) := by
    have ⟨hPB, _⟩ := χ.spec (initial.to (⊤_ 𝓒)) (Ω := Ω) <| «CH.3».«§3.16».«Theorem 1».«(4)» _
    have : terminal.from (⊥_ 𝓒) = initial.to (⊤_ 𝓒) := by ext
    rw [this] at hPB
    apply hPB
  apply «Lemma» («CH.3».«§3.6».Excercises.«3» 𝓒 _) («CH.3».«§3.6».Excercises.«3» 𝓒 _) this
...
theorem «(i)» [ElementaryTopos Ω] [WellPointed 𝓒] : surjective f ↔ Epi f := by
  ...
  have hpEpi : Epi p := «§5.3».«Fact 1» (IsPullback.of_hasPullback y f).flip hepi
